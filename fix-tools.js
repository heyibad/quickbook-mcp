...
files.forEach(file => {
  ...
  const errorPattern = /return\s*{\s*content:\s*\[\s*{\s*type:\s*"text"\s*as\s*const,\s*text:\s*`Error[^`]*:\s*\$\{response\.error\}`[^}]*}\s*\]\s*,?\s*};/g;
  ...
  const successPattern1 = /return\s*{\s*content:\s*\[\s*{\s*type:\s*"text"\s*as\s*const,\s*text:\s*JSON\.stringify\((?:response\.result|vendor|customer|[a-z]+)\)[^}]*}\s*\]\s*,?\s*};/g;
  content = content.replace(successPattern1, (match) => {
    if (match.includes('structured:')) return match;
    
    const dataVar = match.match(/JSON\.stringify\(([\w.]+)\)/)?.[1] || 'response.result';
  ...
  const successPattern2 = /return\s*{\s*content:\s*\[\s*{\s*type:\s*"text"\s*as\s*const,\s*text:\s*`[^`]+`\s*},\s*{\s*type:\s*"text"\s*as\s*const,\s*text:\s*JSON\.stringify\(response\.result\)[^}]*}\s*\]\s*,?\s*};/g;
