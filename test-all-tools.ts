...
async function testAllTools(config: TestConfig) {
  ...
  try {
    ...
    if (!result.isError && result.result) {
      ...
      logResult("Search Accounts", "FAIL", result.error || "Unknown error", duration);
    }
  } catch (error: any) {
  ...
  try {
    ...
    if (!result.isError && result.result) {
      ...
      logResult("Search Customers", "FAIL", result.error || "Unknown error", duration);
    }
  } catch (error: any) {
  ...
  try {
    ...
    if (!result.isError && result.result) {
      ...
      logResult("Search Vendors", "FAIL", result.error || "Unknown error", duration);
    }
  } catch (error: any) {
  ...
  try {
    ...
    if (!result.isError && result.result) {
      ...
      logResult("Search Items", "FAIL", result.error || "Unknown error", duration);
    }
  } catch (error: any) {
  ...
  try {
    ...
    if (!result.isError && result.result) {
      ...
      logResult("Search Employees", "FAIL", result.error || "Unknown error", duration);
    }
  } catch (error: any) {
  ...
  try {
    ...
    if (!result.isError && result.result) {
      ...
      logResult("Search Bills", "FAIL", result.error || "Unknown error", duration);
    }
  } catch (error: any) {
  ...
  try {
    ...
    if (!result.isError && result.result) {
      ...
      logResult("Search Bill Payments", "FAIL", result.error || "Unknown error", duration);
    }
  } catch (error: any) {
  ...
  try {
    ...
    if (!result.isError && result.result) {
      ...
      logResult("Search Estimates", "FAIL", result.error || "Unknown error", duration);
    }
  } catch (error: any) {
  ...
  try {
    ...
    if (!result.isError && result.result) {
      ...
      logResult("Search Invoices", "FAIL", result.error || "Unknown error", duration);
    }
  } catch (error: any) {
  ...
  try {
    ...
    if (!result.isError && result.result) {
      ...
      logResult("Search Journal Entries", "FAIL", result.error || "Unknown error", duration);
    }
  } catch (error: any) {
  ...
  try {
    ...
    if (!result.isError && result.result) {
      ...
      logResult("Search Purchases", "FAIL", result.error || "Unknown error", duration);
    }
  } catch (error: any) {
  ...
  if (prereqData.bills.length > 0) {
    try {
      ...
      const start = Date.now();
      const result = await runInContext(config, () => getQuickbooksBill(billId));
      const duration = Date.now() - start;
      if (!result.isError) {
        logResult("Get Bill", "PASS", `Retrieved bill ${billId}`, duration);
      } else {
        logResult("Get Bill", "FAIL", result.error || "Unknown error", duration);
      }
    } catch (error: any) {
  ...
  if (prereqData.billPayments.length > 0) {
    try {
      ...
      const start = Date.now();
      const result = await runInContext(config, () => getQuickbooksBillPayment(id));
      const duration = Date.now() - start;
      if (!result.isError) {
        logResult("Get Bill Payment", "PASS", `Retrieved bill payment ${id}`, duration);
      } else {
        logResult("Get Bill Payment", "FAIL", result.error || "Unknown error", duration);
      }
    } catch (error: any) {
  ...
  if (prereqData.estimates.length > 0) {
    try {
      ...
      const start = Date.now();
      const result = await runInContext(config, () => getQuickbooksEstimate(id));
      const duration = Date.now() - start;
      if (!result.isError) {
        logResult("Get Estimate", "PASS", `Retrieved estimate ${id}`, duration);
      } else {
        logResult("Get Estimate", "FAIL", result.error || "Unknown error", duration);
      }
    } catch (error: any) {
  ...
  if (prereqData.journalEntries.length > 0) {
    try {
      ...
      const start = Date.now();
      const result = await runInContext(config, () => getQuickbooksJournalEntry(id));
      const duration = Date.now() - start;
      if (!result.isError) {
        logResult("Get Journal Entry", "PASS", `Retrieved journal entry ${id}`, duration);
      } else {
        logResult("Get Journal Entry", "FAIL", result.error || "Unknown error", duration);
      }
    } catch (error: any) {
      logResult("Get Journal Entry", "FAIL", error.message, 0);
    }
  } else {
    logResult("Get Journal Entry", "SKIP", "No journal entries available", 0);
  }
  ...
  if (prereqData.purchases.length > 0) {
    try {
      ...
      const start = Date.now();
      const result = await runInContext(config, () => getQuickbooksPurchase(id));
      const duration = Date.now() - start;
      if (!result.isError) {
        logResult("Get Purchase", "PASS", `Retrieved purchase ${id}`, duration);
      } else {
        logResult("Get Purchase", "FAIL", result.error || "Unknown error", duration);
      }
    } catch (error: any) {
  ...
  if (prereqData.vendors.length > 0) {
    try {
      ...
      const start = Date.now();
      const result = await runInContext(config, () => getQuickbooksVendor(id));
      const duration = Date.now() - start;
      if (!result.isError) {
        logResult("Get Vendor", "PASS", `Retrieved vendor ${id}`, duration);
      } else {
        logResult("Get Vendor", "FAIL", result.error || "Unknown error", duration);
      }
    } catch (error: any) {
  ...
  if (prereqData.invoices.length > 0) {
    try {
      ...
      const start = Date.now();
      const result = await runInContext(config, () => readQuickbooksInvoice(id));
      const duration = Date.now() - start;
      if (!result.isError) {
        logResult("Read Invoice", "PASS", `Read invoice ${id}`, duration);
      } else {
        logResult("Read Invoice", "FAIL", result.error || "Unknown error", duration);
      }
    } catch (error: any) {
  ...
  if (prereqData.items.length > 0) {
    try {
      ...
      const start = Date.now();
      const result = await runInContext(config, () => readQuickbooksItem(id));
      const duration = Date.now() - start;
      if (!result.isError) {
        logResult("Read Item", "PASS", `Read item ${id}`, duration);
      } else {
        logResult("Read Item", "FAIL", result.error || "Unknown error", duration);
      }
    } catch (error: any) {
  ...
  if (prereqData.customers.length > 0) {
    try {
      ...
      const duration = Date.now() - start;
      if (!result.isError) {
        logResult("Update Customer", "PASS", `Updated customer ${customer.Id}`, duration);
      } else {
        logResult("Update Customer", "FAIL", result.error || "Unknown error", duration);
      }
    } catch (error: any) {
  ...
  if (prereqData.vendors.length > 0) {
    try {
      ...
      const duration = Date.now() - start;
      if (!result.isError) {
        logResult("Update Vendor", "PASS", `Updated vendor ${vendor.Id}`, duration);
      } else {
        logResult("Update Vendor", "FAIL", result.error || "Unknown error", duration);
      }
    } catch (error: any) {
  ...
  if (prereqData.accounts.length > 0) {
    try {
      ...
      const duration = Date.now() - start;
      if (!result.isError) {
        logResult("Update Account", "PASS", `Updated account ${account.Id}`, duration);
      } else {
        logResult("Update Account", "FAIL", result.error || "Unknown error", duration);
      }
    } catch (error: any) {
  ...
  try {
    ...
    if (!result.isError) {
      ...
      logResult("Create Customer", "FAIL", result.error || "Unknown error", duration);
    }
  } catch (error: any) {
  ...
  try {
    ...
    if (!result.isError) {
      ...
      logResult("Create Vendor", "FAIL", result.error || "Unknown error", duration);
    }
  } catch (error: any) {
